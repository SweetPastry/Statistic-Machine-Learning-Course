...
$$
P\left[ \exists f\in \mathcal{F} : R\left( f \right) -\hat{R}\left( f \right) >\sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)} \right] =\bigcup_{f\in \mathcal{F}}{P\left[ R\left( f \right) -\hat{R}\left( f \right) >\sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)} \right]}.
$$

利用 Hoeffding 不等式得到加强的命题

$$
\exp \left( -\frac{2d^2}{\sum\limits_{i=1}^n{\left( b_i-a_i \right) ^2}}\frac{1}{2d}\left( \log d+\log \frac{1}{\delta} \right) \right) \leqslant \delta.
$$

利用误差的取值范围

$$
\exp \left( - \log d-\log \frac{1}{\delta}  \right) \leqslant \delta .
$$
