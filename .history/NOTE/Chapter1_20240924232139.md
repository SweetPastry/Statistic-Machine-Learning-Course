...
$$
P\left[ \forall f\in \mathcal{F} :\ R\left( f \right) -\hat{R}\left( f \right) \leqslant \varepsilon \left( d,N,\delta \right) \right] \geqslant 1-\sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)}.
$$

即证明

$$
P\left[ \exists f\in \mathcal{F} : R\left( f \right) -\hat{R}\left( f \right) \geqslant \varepsilon \left( d,N,\delta \right) \right] > \sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)}.
$$

利用 Hoeffding 不等式得到加强的命题

$$
\exp \left( -\frac{d^2\varepsilon ^2\left( d,N,\delta \right)}{\sum\limits_{i=1}^n{\left( b_i-a_i \right) ^2}} \right) \leqslant \sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)}
$$
