...
$$
P\left[ \exists f\in \mathcal{F} : R\left( f \right) -\hat{R}\left( f \right) \geqslant \sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)} \right] \leqslant \delta .
$$

利用 Hoeffding 不等式得到加强的命题

$$
\exp \left( -\frac{d^2}{\sum_{i=1}^n{\left( b_i-a_i \right) ^2}}\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right) \right) \leqslant \sqrt{\frac{1}{2N}\left( \log d+\log \frac{1}{\delta} \right)}.
$$
