...
$$
p\left( H \right) =\mathrm{Beta}\left( \alpha ,\beta \right) .
$$

根据 Bayes 公式

$$
p\left( H|D \right) =\frac{p\left( D|H \right) p\left( H \right)}{p\left( D \right)}=\frac{C_{n}^{k}p^k\left( 1-p \right) ^{n-k}}{p\left( D \right)}\mathrm{Beta}\left( \alpha ,\beta \right) =\frac{C_{n}^{k}}{p\left( D \right)}\mathrm{Beta}\left( \alpha +k,\beta +n-k \right) 
$$
