...
得证. 此段证明, 在运用 Hoeffding 引理处, 认为泛化误差 $\hat{R}$ 为训练误差 $R$ 的期望, 我个人认为这种观点不太正确, 因为模型在训练集上训练, 训练误差是被降得很小得, 而泛化误差是考虑全部的数据, 误差是相对大的. 此处, 应该把测试误差 $R_{\mathrm{test}}$ 的期望当作是泛化误差的估计, 即多进行一步放缩
